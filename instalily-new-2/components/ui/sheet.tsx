...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-50 bg-black/80 opacity-0 transition-opacity duration-300 data-[state=open]:opacity-100",
      className
    )}
    {...props}
    ref={ref}
  ...
  "fixed z-50 gap-4 bg-background p-6 shadow-lg transition-transform duration-300",
  {
    variants: {
      side: {
        ...
        right: "inset-y-0 right-0 h-full w-3/4 border-l translate-x-full data-[state=open]:translate-x-0 sm:max-w-sm",
      },
    },
    defaultVariants: {
      side: "right",
