...
interface LogoSectionProps {
  profileId: string
  profileName: string  // Add profile name for better logging
}
...
const LogoSection = ({ profileId, profileName }: LogoSectionProps) => {
  ...
  useEffect(() => {
    // Debug log for profile ID
    console.log(`LogoSection mounted for ${profileName} (ID: ${profileId})`);
    
    // We'll use a shared loader for all logos
    ...
    window.profileLogosPromise
      .then(allLogos => {
        ...
        // If profile has logos, show them in detail
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    // In production, handle silently
    if (loading || error) {
      return null;
    }
  }
  
  // Always return null for empty logos, regardless of environment
...
// Add the type definition for the window object
declare global {
  interface Window {
      ...
      <div className="aspect-[3/4] bg-white shadow-lg rounded-lg select-none relative overflow-hidden">
        <div className="absolute inset-0 rounded-lg overflow-hidden">
          <img 
            src={imgUrl} 
            alt={name}
            className="object-cover object-center w-full h-full select-none grayscale hover:grayscale-0 transition-all duration-300"
